...
<div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
    <h2 class="text-xl font-semibold text-gray-800 mb-6">Edit Produk: {{ $product->name }}</h2>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            @if($product->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/products/' . $product->image) }}" alt="Current Product Image" class="w-32 h-32 object-cover rounded shadow-md">
                    <p class="text-gray-500 text-xs mt-1">Gambar saat ini: {{ $product->image }}</p>
                </div>
            @endif
        ...
        <div class="mb-4">
            ...
            <input type="text" name="material" id="material" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('material') border-red-500 @enderror" value="{{ old('material', $product->material) }}" placeholder="Contoh: Cotton, Wool">
            @error('material')
                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="5" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500 @error('description') border-red-500 @enderror" placeholder="Masukkan deskripsi produk">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
            @enderror
        </div>
