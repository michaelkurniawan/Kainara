...
class ArtisanRegistrationController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'upload_photo' => 'required|array', // Now 'required|array'
            'upload_photo.*' => 'image|mimes:jpeg,png,jpg,gif,webp|max:4096',
            'video_link' => 'nullable|url',
        ]);

        try {
            // Create a new ArtisanProfile record
            ...
            // Handle portfolio submissions as a single entry with multiple photos
            $photoPaths = [];
            ...
            // Create a single portfolio record related to the profile
            $profile->portfolios()->create([
                ...
                'photo_path' => json_encode($photoPaths), // Store the array as a JSON string
                'video_link' => $validatedData['video_link'],
            ]);

        } catch (\Exception $e) {
            Log::error('Failed to save artisan registration: ' . $e->getMessage());
